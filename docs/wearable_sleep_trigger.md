...
- `sleeptrigger/HealthConnectSleepTriggerSource`：读取最近 Health Connect 睡眠会话，优先读取 Mi Fitness（`com.xiaomi.wearable`）和 Zepp Life（`com.xiaomi.hm.health`）写入的数据；若没有小米来源记录会回退读取全部来源，主要使用已同步的 `SleepEnded`；`SleepStarted` 不作为可靠实时开麦依据。
...
2. 在本应用设置页授权 Health Connect 睡眠读取和后台读取权限；仅有睡眠读取时只能手动检查最近睡眠，后台周期检查和自动停录仍需要后台读取权限。如果 Health Connect 未安装、需更新或授权页不可用，可点击“打开 Health Connect 设置/安装更新”进入系统设置、应用详情或商店兜底入口。
