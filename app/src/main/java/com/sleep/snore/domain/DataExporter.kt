...
internal fun csv(value: Any?): String {
    ...
    val sanitized = if (text.dropWhile { it.isWhitespace() }.firstOrNull() in CSV_FORMULA_PREFIXES) "'$text" else text
...
class DataExporter @Inject constructor(
    ...
    suspend fun exportAllEventsCsv(): File? = withContext(Dispatchers.IO) {
        ...
        file.utf8Writer().use { writer ->
            writer.write("\uFEFF")
            writer.write("记录ID,时间,持续(ms),峰值dB,平均dB,主导频率Hz,类型,AI标签,音频文件,文件大小(bytes)\n")
            events.forEach { e ->
                writer.writeCsvRow(
                    e.recordId,
                    ...
                    e.snoreType,
                    e.aiTypeLabel,
                    e.audioFilePath.takeIf { it.isNotBlank() }?.let { File(it).name }.orEmpty(),
