...
class SleepRecordingService : Service() {
    ...
    private fun createNotificationChannel() {
        val channel = android.app.NotificationChannel(
            CHANNEL_ID, "÷ýÉùÂ¼ÖÆ", android.app.NotificationManager.IMPORTANCE_LOW
        ).apply {
            description = "Ë¯Ãß÷ýÉùÂ¼ÖÆ·þÎñÍ¨Öª"
        }
    ...
    private fun startForegroundNotification() {
        ...
        val notification = NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle("ÕýÔÚÂ¼ÖÆË¯Ãß÷ýÉù")
            .setContentText("÷ýÉùÆ¬¶Î: 0 ¸ö")
    ...
    private fun startSnoreDetection() {
        ...
        snoreDetector = SnoreDetector(object : SnoreDetector.SnoreCallback {
            override fun onSnoreStarted(timestamp: Long, db: Double) {
                Log.d(TAG, "÷ýÉù¿ªÊ¼: $timestamp, ${String.format("%.1f", db)}dB")
            }
                ...
                serviceScope.launch {
                    try {
                        val event = SnoreEventEntity(
                            ...
                            aiTypeLabel = "${currentSnoreType.label}÷ýÉù"
                        )
                        pendingEvents.add(event)
                    } catch (e: Exception) {
                        Log.e(TAG, "±£´æ÷ýÉùÊÂ¼þÊ§°Ü", e)
                    }
                }
            }
        })
    ...
    private fun updateNotification() {
        val notification = NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle("ÕýÔÚÂ¼ÖÆË¯Ãß÷ýÉù")
            .setContentText("÷ýÉùÆ¬¶Î: $snoreEventCount ¸ö")
            .setSmallIcon(android.R.drawable.ic_btn_speak_now)
            .setOngoing(true)
            .build()
